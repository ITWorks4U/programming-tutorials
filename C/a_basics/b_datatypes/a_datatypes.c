//  display the amount of bytes for each known basic data type in C
...
int main(void) {
    ...
    // NOTE: On a Windows system, "%lu" gives you a warning
    //       on compile time, because sizeof() does not offer an
    //       unique return value for each system.
    //
    //       Windows: usually sizeof = unsigned long long => %llu
    //       UNIX:    usually sizeof = unsigned long      => %lu
